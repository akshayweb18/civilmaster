...
export const DifficultyCard: React.FC<DifficultyCardProps> = ({
  ...
  return (
    <button
      onClick={onClick}
      className={`
        group relative w-full p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] border-2 transition-all duration-300 text-left flex flex-col no-anim min-h-[12rem]
        ${isSelected 
    ...
    >
      ...
      <div className="flex items-start justify-between mb-6 md:mb-8">
        <div className={`w-14 h-14 md:w-16 md:h-16 rounded-2xl flex items-center justify-center text-3xl md:text-4xl shadow-xl transition-all ${
        ...
        {isSelected && (
          <div className={`px-3 py-1.5 bg-${theme.color}-500 text-white text-[9px] font-black uppercase tracking-widest rounded-full shadow-lg`}>
            Tier Active
          </div>
        )}
      </div>
      ...
      <div className="space-y-2 flex-1">
        <div className="flex items-center gap-2">
          <h3 className={`text-2xl md:text-3xl font-display font-black uppercase tracking-tighter leading-none italic ${isSelected ? "text-white" : "text-slate-900"}`}>
            {difficulty}
          </h3>
          {isSelected && <div className={`w-2 h-2 rounded-full bg-${theme.color}-500 shadow-[0_0_10px_rgba(0,0,0,0.5)]`} />}
        </div>
        <p className={`text-[10px] md:text-sm font-bold uppercase tracking-wide leading-relaxed ${isSelected ? "text-slate-400" : "text-slate-500"}`}>
          {description}
        </p>
      </div>
      ...
      {/* Footer Decoration */}
      {!isSelected && (
        <div className="mt-6 md:mt-8 pt-4 border-t border-slate-50 group-hover:border-slate-200 flex items-center justify-between">
